...
int main(int argc, char const *argv[])
{
  ...
  for(int i = 0; i < imagePoints1.size(); i++ )
  {
        ...
        undistortPoints(imgpt[0], imgpt[0], K1, D1, Mat(), K1);
        computeCorrespondEpilines(imgpt[0], 1, F, lines[0]);
        imgpt[1] = Mat(imagePoints2[i]);
        undistortPoints(imgpt[1], imgpt[1], K2, D2, Mat(), K1);
        ...
        cout << i << " epipolar err = " << (err - err_img)/npt << endl;
        npoints += npt;
        err_img = err;
    }
